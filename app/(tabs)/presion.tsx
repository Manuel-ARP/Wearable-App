...
const dayPressures = dayLabels.map((_, i) => ({
  sys: 118 + Math.sin(i / 3) * 6 + (i % 6 === 0 ? 6 : 0),
  dia: 76 + Math.sin(i / 3) * 4 + (i % 6 === 0 ? 4 : 0),
}));
...
export default function PresionScreen() {
  ...
  const chartData = useMemo(
    () => ({
      ...
      datasets: [
        {
          data: current.values.map((p) => p.sys),
          color: () => palette.teal,
          strokeWidth: 3,
        },
        {
          data: current.values.map((p) => p.dia),
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.header}>Presion Arterial</Text>
          <View style={{ width: 22 }} />
        </View>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Tendencia de datos - Presion Arterial (mmHg)</Text>
        ...
        <View style={styles.cardsRow}>
          <View style={styles.infoCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.infoLabel}>Promedio Diario</Text>
              <Text style={styles.infoValue}>{current.avg}</Text>
            </View>
          </View>
          <View style={styles.infoCard}>
            <View style={styles.infoIcon}>
              <Ionicons name="checkmark-done-outline" size={18} color={palette.teal} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.infoLabel}>Presion arterial normal</Text>
              <Text style={styles.infoValue}>{current.normal}</Text>
            </View>
          </View>
        </View>
      </ScrollView>
